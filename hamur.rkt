...
(define total-deaths 0)
(struct grain (harvest store ratfood))
(struct census (pop born diseased starved))

(define (report year cn)
  (printf "YEAR ~a~n" year)
  (printf "POPN\n")
  (printf " POPULATION~n")
  (printf "  TOTAL ~a~n" (census-pop cn))
  (printf "  BIRTHS ~a~n" (census-born cn))
  (printf "  DISEASED ~a~n" (census-diseased cn))
  (printf "  STARVED ~a~n" (census-starved cn))
  )

(define (new-year year popn grain acres planted)
  (define pop (census-pop popn))
  (define born (census-born popn))
  (define diseased (census-diseased popn))
  (define starved (census-starved popn))
  (report year popn)
  (if (<= year 10)
      (let ((feed (distribute-grain grain)))
        (let ((price (+ 1 (random 25))))
          (let ((d-acres (trade-acres acres grain price)))
            (letrec ((planted (plant-crops pop acres grain))
              ...
              (letrec ((diseased (plague pop))
                ...
                (set! total-deaths (+ total-deaths starved diseased))
                (new-year (+ year 1)
                          (census (- (+ pop born) (+ starved diseased)) pop born diseased starved)
                          (grain harvested
                                 (- harvested ratfood) ratfood)
                          (+ acres d-acres)
                          (- grain (* d-acres price))
                          planted))))))
      (end-game popn acres total-deaths)))
...
(define (trade-acres acres grain price)
  (let ((buy
...
(define (hammurabi)
  (let ((popn (census 100 0 0 0))
        (grain (grain 3000 2800 200)))
    (new-year 1 popn grain 100 100)))
